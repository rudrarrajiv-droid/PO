...
type NormalizedRow = {
  ...
  total_ply: number | null;
  scrap: number | null;
  raw_data: Record<string, unknown>;
};

const FIREBASE_PROJECT_ID = "job-card-cd56f";
const FIRESTORE_COLLECTION = "dc_records";
const TABLE_NAME = "public.dc_records";
...
function normalizeDoc(
  ...
  const recordDate = typeof data.date === "string" ? data.date : null;
  const totalPly = typeof data.totalPly === "number" ? data.totalPly : null;
  const scrap = typeof data.scrap === "number" ? data.scrap : null;
...
async function loadCreateTableSql(): Promise<string> {
  ...
  const sqlPath = path.join(currentDir, "sql", "create_firestore_dc_records.sql");
...
async function run(): Promise<void> {
  ...
  const upsertSql =
    ...
    TABLE_NAME +
    " (" +
    "firestore_document_id, record_date, total_ply, scrap, raw_data, imported_at, synced_at" +
    ") values ($1, $2, $3, $4, $5::jsonb, now(), now()) " +
    "on conflict (firestore_document_id) do update set " +
    "record_date = excluded.record_date, " +
    "total_ply = excluded.total_ply, " +
