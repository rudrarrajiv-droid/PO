...
async function run() {
  ...
  let totalSKStockWeight = 0;
  let totalSKStockValue = 0;
  ...
  for (let r = 5; r <= 114; r++) {
    ...
    if (reelNumber && reelNumber !== 'undefined') {
      ...
      if (!isNaN(balance)) totalSKStockWeight += balance;
      if (!isNaN(val)) totalSKStockValue += val;
