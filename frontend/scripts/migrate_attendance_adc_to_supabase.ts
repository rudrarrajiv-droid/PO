...
const FIRESTORE_COLLECTION = "attendance";
const TABLE_NAME = "public.attendance";
...
function normalizeDoc(
  ...
  const attendanceDate = typeof data.date === "string" ? data.date : null;
  const present = typeof data.present === "number" ? data.present : null;
  const otHours = typeof data.otHours === "number" ? data.otHours : null;
  const refreshment =
    typeof data.refreshment === "number" ? data.refreshment : null;
  const perDayAmount =
    typeof data.perDayAmount === "number" ? data.perDayAmount : null;
  const otAmount = typeof data.otAmount === "number" ? data.otAmount : null;
...
async function loadCreateTableSql(): Promise<string> {
  ...
  const sqlPath = path.join(currentDir, "sql", "create_firestore_attendance.sql");
...
async function run(): Promise<void> {
  ...
  const upsertSql =
    ...
    TABLE_NAME +
    " (" +
    "firestore_document_id, employee_id, attendance_date, present, ot_hours, refreshment, per_day_amount, ot_amount, month, created_by, updated_by, created_at, updated_at, raw_data, imported_at, synced_at" +
    ...
    "refreshment = excluded.refreshment, " +
    "per_day_amount = excluded.per_day_amount, " +
